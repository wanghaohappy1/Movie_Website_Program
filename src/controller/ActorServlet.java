...
public class ActorServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		if (castDao.findcast(character1) == true
				&& actorDao.findActorId(actor1) != 0) {
			dispatcher = request.getRequestDispatcher("SearchDetail.jsp");
			dispatcher.forward(request, response);
		} else {
			castDao.create3Cast(actor1Id, movieId, character_1, actor2Id,
					movieId, character_2, actor3Id, movieId, character_3);
			dispatcher = request.getRequestDispatcher("SearchDetail.jsp");
			dispatcher.forward(request, response);
		}
